<?php
    ...
    if (KSESTEST_VER == 0) {
        ...
        displayPage(
            ["title" => "Error running KSES test", "message" => $message]
        );
    ...
    if (file_exists($include_file) && is_readable($include_file)) {
        ...
        displayPage(
            ['title' => 'Unable to include ' . $include_file, 'message' => $message]
        );
    ...
    displayPage(
        ['title' => 'New Test', 'message' => $test_text]
    );

    function test1_kses(&$myKses)
    {
        $out = [output_hr(), "Testing current configuration"];
        ...
        $out = array_merge($out, [output_ul($out_li)]);
    ...
    function displayPage($data = [])
    {
        ...
        $message = ($data['message'] == '') ? ['No message'] : $data['message'];
    ...
    function output_ul($data = [], $padding = "")
    {
    ...
    function test1_protocols(&$myKses)
    {
        ...
        $myKses->AddProtocols(["proto1", "proto2:", "proto3"]);   // Add a list of protocols
        ...
        $myKses->RemoveProtocols(["mystery", "anarchy:"]);
        ...
        $myKses->SetProtocols(["https", "gopher", "news"]);
        $set_protocol  = "\t\t\t<ol>\n";
        ...
        //	Invisible reset
        $myKses->SetProtocols(["http", "proto1", "proto2", "proto9"]);
            ...
            foreach ($out_elems as $tag => $attr_data) {
                ...
                if (isset($attr_data) && is_array($attr_data) && count($attr_data) > 0) {
                    ...
                    if (count($attr_test_li) > 0) {
                        ...
                        $out_li_elems = ["$elem_text$allow$attr_test_li"];
                    }
                }
