...
{
    return html_entity_decode(strtr($text, ['&apos;' => '\'']), ENT_QUOTES, 'UTF-8');
}
...
function _error_report($error)
{
    return [
        ...
        'line' => $error->line,
        'col' => $error->column
    ] + ($error->file == '' ? [] : ['directory' => dirname($error->file), 'file' => basename($error->file)]);
}
...
class php4DOMDocument extends php4DOMNode
{
    ...
    public function add_root($name)
    {
        ...
        return new php4DOMElement(
            $this->myDOMNode->appendChild($this->myDOMNode->createElement($name)),
    ...
    public function create_element_ns($uri, $name, $prefix = null)
    {
        ...
        return new php4DOMElement($this->myDOMNode->createElementNS(
            $uri,
    ...
    public function create_processing_instruction($target, $data = '')
    {
...
class php4DOMElement extends php4DOMNode
{
    ...
    public function set_name($name)
    {
        if ($this->myDOMNode->prefix == '') {
            ...
            $newNode = $this->myDOMNode->ownerDocument->createElementNS(
                $this->myDOMNode->namespaceURI,
...
class php4DOMNode
{
    ...
    public function __get($name)
    {
        switch ($name) {
            ...
            default:
                ...
                trigger_error(
                    'Undefined property: ' . get_class($this) . '::$' . $name . ' [' . $myErrors[0]['file'] . ':' . $myErrors[0]['line'] . ']',
                    E_USER_NOTICE
                );
                return false;
        }
    ...
    public function add_child($newnode)
    {
    ...
    {
        return self::_newDOMElement(
            $this->myDOMNode->appendChild($this->_importNode($newnode)),
            $this->myOwnerDocument
        );
    }
    ...
    {
        return self::_newDOMElement(
            $this->myDOMNode->parentNode->appendChild($this->_importNode($newnode)),
            $this->myOwnerDocument
        );
    }
    ...
    public function dump_node($node = null)
    {
    ...
    {
        return self::_newDOMElement($this->myDOMNode->insertBefore(
            $this->_importNode($newnode),
            $refnode == null ? null : $refnode->myDOMNode
        ), $this->myOwnerDocument);
    }
    ...
    {
        return self::_newDOMElement(
            $this->myDOMNode->replaceChild($this->_importNode($newnode), $oldnode->myDOMNode),
            $this->myOwnerDocument
        );
    }
    ...
    {
        return self::_newDOMElement($this->myDOMNode->parentNode->replaceChild(
            $this->_importNode($newnode),
            $this->myDOMNode
        ), $this->myOwnerDocument);
    }
    ...
    {//Contributions by Daniel Walker 2006-09-08
        ...
        if ($nsprefix == null) {
            ...
            if ($this->myDOMNode->nodeType === XML_ATTRIBUTE_NODE) {
                if (($prefix != null) && $this->myDOMNode->ownerElement->hasAttributeNS(
                    'http://www.w3.org/2000/xmlns/',
                        $nsprefix
                ) &&
                ...
                $this->myDOMNode->ownerElement->setAttributeNS(
                    'http://www.w3.org/2000/xmlns/',
                    'xmlns:' . $nsprefix,
                    $uri
                );
    ...
    {
        return $this->myOwnerDocument === $newnode->myOwnerDocument ? $newnode->myDOMNode : $this->myOwnerDocument->myDOMNode->importNode(
            $newnode->myDOMNode,
            true
        );
...
class php4DOMXPath
{
    ...
    public function xpath_eval($eval_str, $contextnode = null)
    {
        if (method_exists($this->myDOMXPath, 'evaluate')) {
            $xp = isset($contextnode) ? $this->myDOMXPath->evaluate(
                $eval_str,
                $contextnode->myDOMNode
            ) : $this->myDOMXPath->evaluate($eval_str);
        } else {
            $xp = isset($contextnode) ? $this->myDOMXPath->query(
                $eval_str,
                $contextnode->myDOMNode
            ) : $this->myDOMXPath->query($eval_str);
        }
...
if (extension_loaded('xsl')) {//See also: http://alexandre.alapetite.net/doc-alex/xslt-php4-php5/
    ...
    class php4DomXsltStylesheet
    {
        ...
        public function php4DomXsltStylesheet($dom_document)
        {
